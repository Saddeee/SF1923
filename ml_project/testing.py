...
# x_input = np.array([1, x_input])

t = x_input
w = [-1.2, 0.9]
t = w[0] + t*w[1]
ones = [(1, x) for x in x_input]

# t_x_tuple = (t, ones)


# likelihood = [multivariate_normal(np.transpose(w)*xi, cov).pdf(t) for (t, xi) in t_x_tuple] 
# likelihood = np.prod(likelihood)

# Create a tuple of paired elements from t and x_input
t_x_tuple = [(t_val, np.array([1, x_val])) for t_val, x_val in zip(t, x_input)]

# Compute the likelihood
likelihood = [np.norm(np.dot(w, xi), cov) for (t_val, xi) in t_x_tuple]
likelihood = np.prod(likelihood)
